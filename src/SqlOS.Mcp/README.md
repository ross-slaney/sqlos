...
app.MapGet("/api/gardens", ...).RequireAuthorization();
app.Run();
```

Declaring the MCP surface makes SqlOS:

- register scheme/policy `SqlOS.Mcp` with audience `{Origin}/mcp` and require it on the mapped MCP endpoints,
...
- register the MCP SDK server (stateless Streamable HTTP), apply your `configure` callback to the
  SDK builder unchanged, and map it at startup,
- record a SqlOS audit event per tool call (tool name, subject, client, outcome; never arguments or tokens).

Lock your own API routes with `RequireAuthorization()`. `ConfigureApplication` retains this hosting setup while you register multiple clients explicitly.
