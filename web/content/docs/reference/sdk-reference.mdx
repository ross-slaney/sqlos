...
The configured `ISqlOSEmailSender` performs delivery. When Azure Communication Services is not your provider, register a custom implementation after `AddSqlOS` so it becomes the single service resolved by the sender pipeline. Built-in auth templates use the `auth.*` keys and are normally invoked by the corresponding AuthServer workflow. See [Transactional Email](/docs/guides/transactional-email) for provider configuration, retention, privacy, and built-in templates.
...
See [Calendar Integration](/docs/guides/calendar-integration) for provider scopes, scheduler configuration, conflict policy, and dashboard operations.
