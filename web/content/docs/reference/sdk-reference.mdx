...
### Direct SAML handoff-code exchange

`SqlOSAuthService.ExchangeCodeAsync` consumes the temporary `auth_code` produced by `SqlOSSamlService.CreateAuthorizationUrlAsync` and its request-token callback flow:
...
This is not the OAuth authorization-code + PKCE exchange. Standards-based clients must call form-encoded `POST /sqlos/auth/token`; custom SAML/OIDC PKCE orchestration uses `SqlOSSsoAuthorizationService` or `SqlOSOidcBrowserAuthService` with `SqlOSPkceExchangeRequest`.
