...
>
  New to SqlOS? Complete [Getting Started](/docs/getting-started) first, then browse by capability from the [documentation home](/docs). Use this page when you need the exact signature for a specific service method.
</Banner>
...
See [CLI OAuth](/docs/guides/terminal-auth).
...
The same restriction applies to password, Email OTP, and phone OTP self-service signup. To add a user to an existing organization, use the [invitation flow](/docs/guides/invite-only-b2b-onboarding) or trusted backend provisioning with `CreateUserAsync` followed by `CreateMembershipAsync`.
...
| `request.AutoLinkByEmail` | `bool` | Require existing members to sign in through this SSO connection during home realm discovery. First-time binding of an existing global user still requires the organization to own the asserted email domain. |
...
Provider setup is covered end to end in [Google OIDC](/docs/authserver/google-oidc), [Microsoft OIDC](/docs/authserver/microsoft-oidc), [Apple OIDC](/docs/authserver/apple-oidc), [GitHub OAuth](/docs/guides/github-oidc), and [Custom OIDC](/docs/authserver/custom-oidc). See [OIDC social login](/docs/authserver/oidc-auth) for the shared browser flow.
...
See [GitHub OIDC](/docs/guides/github-oidc).
...
<Callout type="warning" title="Prefer the policy-aware browser flows">
  For application login, use SqlOS's hosted authorization endpoints or [build a headless custom login UI](/docs/guides/custom-login-ui). Those flows preserve organization selection, MFA, PKCE, state, nonce, and the normal authorization handoff. Use `CompleteAuthorizationAsync` directly only when you are building a trusted server-side identity broker. If that broker later calls `CreateSessionTokensForUserAsync`, it must first enforce organization membership and the required MFA policy itself; direct token creation bypasses the normal login state machine. Token creation still applies SqlOS application-access assignments.
</Callout>
...
- Users: `id`, `userName`, `externalId`, and `emails.value`
- Groups: `id`, `displayName`, and `externalId`

User PATCH accepts pathless provider values plus `active`, display/name fields, `userName`, and emails. Group PATCH accepts display-name changes, full member replacement, member add/remove, pathless values, and filtered removal with `members[value eq "..."]`; it returns `204 No Content` unless the caller requests a projected response with `attributes` or `excludedAttributes`. Bulk, sort, ETags, and password changes are not advertised.
...
See [SCIM Directory Sync](/docs/guides/scim-directory-sync) for the exact filter and PATCH profile, validation commands, error behavior, dashboard workflow, token rotation, and tenant-isolated deprovisioning. See [SCIM group mapping](/docs/fga/scim-group-mapping) for FGA behavior.
