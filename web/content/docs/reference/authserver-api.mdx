...
SCIM provisioning endpoints are mounted separately from the admin API and authenticate with an organization-scoped bearer token:

```text
/sqlos/scim/v2/Users
/sqlos/scim/v2/Groups
```
...
| `POST /organizations/{organizationId}/scim-connections` | Create a SCIM connection |
| `GET /scim-connections/{connectionId}` | Read setup URLs and token metadata |
| `POST /scim-connections/{connectionId}/token/rotate` | Generate a new bearer token |
...
| `PUT /scim-mappings/{mappingId}` | Update a mapping rule |
| `GET /scim-connections/{connectionId}/sync-events` | Read recent SCIM sync activity |

See [SCIM Directory Sync](/docs/authserver/scim-directory-sync) for the full endpoint list, SDK calls, dashboard workflow, and FGA group mapping behavior.
