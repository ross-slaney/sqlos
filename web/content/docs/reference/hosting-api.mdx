...
| `Api` | `string?` | Absolute path of the protected REST surface, for example `/api`. SqlOS validates bearer tokens for the audience `{Origin}{Api}` on mapped endpoints under that path and serves the RFC 9728 document at `/.well-known/oauth-protected-resource`. |
| `Mcp` | `string?` | Absolute path of the protected MCP surface, for example `/mcp`. SqlOS validates tokens for `{Origin}{Mcp}` on mapped endpoints under that path, serves `/.well-known/oauth-protected-resource{Mcp}`, and enables client ID metadata documents plus resource indicators. Set it through `app.Mcp(path, configure)` from the `SqlOS.Mcp` package to also host the server. |
...
`AddSqlOS` attaches that validation to mapped endpoints under the declared paths. SqlOS does not add application pipeline middleware and does not handle CORS. Requests to those endpoints that carry no token or a token for another audience receive HTTP `401` with a Bearer challenge that names the surface's `realm` and `resource_metadata` URL. The validated token is available through `HttpContext.GetSqlOSValidatedToken()` and `HttpContext.User`. Inspect `Scope` on that token in the handler when a client's granted ceiling must be checked.
...
`ConfigureApplication` seeds no client. Use explicit client seeds, the administration API, or dashboard records. Declaring MCP still enables CIMD and resource indicators. The [multiple-applications guide](/docs/authserver/multiple-applications) shows how to preserve an existing client's identity when graduating.
...
**Returns:** the validated token stored by a declared `Api` or `Mcp` surface; otherwise `null`.

```csharp
using SqlOS.AuthServer.Extensions;

// Declare app.Api = "/api" in UseSingleApplication. Mapped endpoints under /api are already protected.
...
Application code contains no `AddMcpServer`, `MapMcp`, or hand-written protected-resource document. `AuthServer.Issuer` defaults to `{Origin}/sqlos/auth`.
