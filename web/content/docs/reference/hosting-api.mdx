...
>
  The recommended path has two integration points: derive your EF context from `SqlOSDbContext<TContext>` and call the combined `AddSqlOS` overload. SqlOS maps its auth endpoints at startup. Declared `Api` and `Mcp` paths are audiences. Lock your routes with `RequireAuthorization()`.
</Banner>
...
**Returns:** the validated token stored by the `SqlOS` authentication scheme; otherwise `null`.
...
SqlOS does not map the MCP server. `AuthServer.Issuer` defaults to `{Origin}/sqlos/auth`.
