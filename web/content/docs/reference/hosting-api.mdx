...
| `Mcp` | `string?` | Absolute path of the MCP resource, for example `/mcp`. Registers scheme/policy `SqlOS.Mcp` with audience `{Origin}{Mcp}`, serves `/.well-known/oauth-protected-resource{Mcp}`, and enables client ID metadata documents plus resource indicators. Set it through `app.Mcp(path, configure)` from the `SqlOS.Mcp` package to also host the server. |
