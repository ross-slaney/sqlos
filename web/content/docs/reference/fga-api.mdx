...
| `.SortBy(name, selector, isDefault?)` | `string`, `Expression<Func<T, TKey>>`, `bool` | Register a named sort column. Built-in serializers cover `string`, `int`, `long`, `decimal`, `double`, `DateTime`, `DateTimeOffset`, `DateOnly`, `Guid`, and `bool`. No `CultureInfo` in caller code. |
| `.SortByString(name, selector, isDefault?)` | `string`, `Expression<Func<T, string>>`, `bool` | Alias for `.SortBy` on a string column. |
| `.SortBy(name, selector, serialize, deserialize, isDefault?)` | `string`, `Expression<Func<T, TKey>>`, `Func<TKey, string>`, `Func<string, TKey>`, `bool` | Escape hatch for comparable types without a built-in serializer. |
