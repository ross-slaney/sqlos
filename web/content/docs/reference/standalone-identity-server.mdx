...
| --- | --- | --- |
| Auth host | ASP.NET app that runs `AddSqlOS` with `ConfigureApplication` and explicit client registrations | [Getting Started](/docs/getting-started) |
...
| Application access | Policy for who may use a client application | [Application Access](/docs/guides/multi-app-access) |
| Headless auth | App-owned UI with SqlOS-owned protocol state | [Headless Auth](/docs/guides/custom-login-ui) |
| Device flow | CLI/native flow without a redirect listener | [CLI OAuth](/docs/guides/terminal-auth) |
...
`ConfigureApplication` seeds no client. Use its `Brand`, `Headless`, and `Authorization` methods for host configuration and the registrations below for each relying party. The [multiple-applications guide](/docs/authserver/multiple-applications) includes migration steps and runnable multi-client examples.
...
This separate API uses `Microsoft.AspNetCore.Authentication.JwtBearer` and validates signature, issuer, expiry, and audience through discovery/JWKS. It does not query SqlOS session state on each request. In a process that hosts SqlOS, declared `Api`/`Mcp` surfaces additionally validate the persisted session through SqlOS services; see [token validation](/docs/authserver/token-validation).
...
The app should call [headless endpoints](/docs/guides/custom-login-ui) with the request ID instead of inventing separate login state.
