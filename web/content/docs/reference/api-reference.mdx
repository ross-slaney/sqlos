...
| `scope` | No | Space-separated. SqlOS always intersects the request with the client's registered allow-list and keeps only the overlap (RFC 6749 §3.3). An empty allow-list grants nothing. Unknown requested scopes are dropped, not rejected. `openid` is an ordinary allow-listed string today and does not mint an `id_token`; issue #118 will decide always-allowed vs warning behavior. |
