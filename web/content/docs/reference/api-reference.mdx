...
| `prompt` | No | `login` skips first-party silent reuse of the AuthPage cookie and shows the hosted/headless form. Send this on an explicit Sign in or Sign up. `select_account` behaves like `login` (SqlOS has no account chooser yet). `none` returns `login_required` when no reusable session exists or the session is older than `max_age`. |
| `nonce` | No | Stored with the authorization request and copied onto the authorization code at issuance so token issuance can bind it once OpenID Provider mode ships. |
