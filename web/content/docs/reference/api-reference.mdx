...
`scopes_supported` is the sorted union of scopes that at least one registered client can be granted. It omits reserved OpenID Connect names (`openid`, `profile`, `email`, `offline_access`) until SqlOS issues ID tokens, and it never includes internal `auth:*` credential-type strings. A client's allowlist may still contain those reserved names; requesting them is accepted by silent intersection. A deployment with no clients advertises an empty list. Operator-defined API scopes such as `x.read` appear when a client allowlist includes them.
...
| `scope` | No | Space-separated. SqlOS always intersects the request with the client's registered allow-list and keeps only the overlap (RFC 6749 §3.3). An empty allow-list grants nothing. Unknown requested scopes are dropped, not rejected. `openid` is never always-allowed and does not mint an `id_token` yet. Hosted and headless warn when an allowlisted `openid` is omitted from the grant; issue #118 will add ID token issuance. |
...
Successful token responses use OAuth-style snake-case JSON fields such as `access_token`, `refresh_token`, `token_type`, `expires_in`, and `scope`. `scope` is the granted set after the same silent intersection used by `/authorize`, device authorization, and `client_credentials`. An empty grant is returned as an empty string rather than `invalid_scope`. OAuth errors use an `error` code and may include `error_description`.
