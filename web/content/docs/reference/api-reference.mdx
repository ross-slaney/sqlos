...
| `scope` | No | Space-separated. SqlOS always intersects requested scopes with the client's `AllowedScopes` allowlist and redirects with the granted subset, which may be empty. An empty allowlist grants no requested scopes; it is not a wildcard. |
...
| `invalid_scope` / `invalid_target` | A requested scope is not on the client's allowlist (an empty allowlist rejects every requested scope), or the requested resource is not allowed. |
