...
Programmer mistakes still throw: `HeadlessFlowBusyError`, `HeadlessFlowNotLoadedError`, missing-token preconditions, and the `/token` guard.
...
`credentials: "include"` is required in the browser so the AuthPage session cookie is sent. Omit it on native. Always leave with `window.location.assign(redirectUrl)` — do not hand-parse the URL.
...
PKCE is generated (or accepted) as start input because SqlOS requires it. Pass `generatePkce` when Web Crypto `subtle` is unavailable (for example Expo). After `flow.authorization`, the host OIDC library exchanges the code.
