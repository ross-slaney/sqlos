...
FGA authorizes a subject but does not authenticate the caller. Declare `app.Api = "/api"`, lock the group with `RequireAuthorization()`, and derive the subject from the SqlOS-validated token:
