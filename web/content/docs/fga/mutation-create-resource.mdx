...
FGA authorizes a subject but does not authenticate the caller. Declare `app.Api = "/api"` so mapped endpoints under `/api` are already protected, and derive the subject from the SqlOS-validated token:
...
All endpoints below are mapped on that protected `api` group. Do not accept `subjectId` from a request body, route value, or unvalidated JWT claim.
