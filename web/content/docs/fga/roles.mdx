...
Roles defined in startup are the authorization model. Change them in `options.Fga.Seed` and restart. The dashboard does not create, edit, or delete roles.
...
The Roles page lists all roles with their permission count so you can inspect the seeded model. Assign those roles to subjects on the Grants page.
