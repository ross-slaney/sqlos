...
<Callout type="info" title="Version contract">
  These docs target **SqlOS 7.0.0**, **.NET 9**, **EF Core 9**, and **SQL Server or PostgreSQL**. Use the same package version as the docs; older releases do not include the complete current schema and source/reference contract.
</Callout>
...
builder.AddSqlOS<AppDbContext>(
    ...
    options => options.UseSingleApplication("Acme", app =>
    {
        ...
        app.Mcp("/mcp", mcp => mcp.WithTools<AcmeTools>());   // MCP server (SqlOS.Mcp)
