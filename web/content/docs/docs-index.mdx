...
<Callout type="info" title="Version contract">
  These docs target **SqlOS 7.2.0**, **.NET 9**, **EF Core 9**, and **SQL Server or PostgreSQL**. Use the same package version as the docs; older releases do not include the complete current schema and source/reference contract.
</Callout>
...
<CardGrid cols={2}>
  ...
  <Card title="Protect an API" description="Declare app.Api, RequireAuthorization(), and read the validated token." href="/docs/quickstarts/protect-api" />
...
<CardGrid cols={2}>
  <Card title="Multiple applications" description="Keep the host description with ConfigureApplication and register each client explicitly." href="/docs/authserver/multiple-applications" />
...
The sidebar is grouped by product capability: host, sign-in, tokens, clients, MCP, organizations, directory, authorization, and operations. Start at [Getting Started](/docs/getting-started) or the [runnable examples](/docs/examples).
