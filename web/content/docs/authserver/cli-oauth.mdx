...
| `invalid_target` | The requested resource does not match the client audience. Unknown requested scopes are dropped by silent intersection with the client allow-list; they do not fail the start request. |
...
Drive device verification with [`@sqlos/headless`](/docs/reference/headless-js) (`flow.resume` / `flow.device.resolve` / `approve` / `deny`). The routes below are the HTTP wire contract; client code should use the package. See [Build your own login and signup UI](/docs/guides/custom-login-ui).
...
Do not implement a separate sign-in page for device flow. Treat device verification as a normal SqlOS auth interaction with an extra final approval step. After the existing auth actions complete, the view is `device-approve`; approve it and tokens are released to the polling CLI.
