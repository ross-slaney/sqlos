...
Resource indicators are enabled in the raw multi-client defaults. `UseSingleApplication(...)` deliberately disables them for the simpler one-app model, so enable them explicitly before expecting a supplied `resource` value to control `aud` in that mode.
