...
Headless UIs receive a view model with `view: "consent"` and `consentScopes` (each entry has `scope`, `displayName`, and optional `description`). Drive it with [`@sqlos/headless`](/docs/reference/headless-js) (`flow.consent.approve` / `flow.consent.deny`). The routes `POST /sqlos/auth/headless/consent/approve` and `/consent/deny` are the HTTP wire contract; client code should use the package. See [Build your own login and signup UI](/docs/guides/custom-login-ui).
