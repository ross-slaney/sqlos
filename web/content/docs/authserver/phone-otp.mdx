...
Headless mode still starts with a normal OAuth authorization request. Drive the UI with [`@sqlos/headless`](/docs/reference/headless-js) (`createHeadlessFlow` / `useHeadlessAuth`). The routes below are the HTTP wire contract; client code should use the package.
...
Example sign-in actions. The flow keeps `requestId` and challenge tokens; actions resolve on validation errors — read `error` and `fieldErrors` instead of try/catch:
...
const flow = createHeadlessFlow({
  issuer: "https://identity.example.com/sqlos/auth",
  clientId: "my-app",
...
await flow.phoneOtp.start({ phoneNumber });
...
Signup uses `flow.phoneOtp.signupStart` and `flow.phoneOtp.signupVerify`. Do not mint a session, authorization code, or application cookie in this UI. Always leave with `window.location.assign(redirectUrl)`.
...
SqlOS reserves every applicable phone, account, IP, and client bucket in one distributed rate-limit transaction before it creates a challenge or calls Twilio. Concurrent requests and replicas therefore cannot exceed the lowest overlapping cap. A provider failure or timeout keeps the reserved slot so SMS spend stays bounded; the window expiry restores capacity and prevents a permanent lockout. IP limits depend on `HttpContext.Connection.RemoteIpAddress`. Behind a reverse proxy, configure ASP.NET Core Forwarded Headers with trusted proxies or networks; otherwise every request can appear to come from the proxy or an untrusted client can spoof forwarding headers. See [Production Readiness](/docs/guides/production-readiness#reverse-proxies-and-forwarded-headers).
