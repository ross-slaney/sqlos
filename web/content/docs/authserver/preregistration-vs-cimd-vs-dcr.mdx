...
`app.Mcp = "/mcp"` on a same-host app turns CIMD and resource indicators on for you. Otherwise CIMD is a flag. Resource indicators are already on outside `UseSingleApplication`.
