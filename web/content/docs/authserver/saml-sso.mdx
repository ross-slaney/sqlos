...
For a complete customer IT administrator journey, use [Let customer admins set up enterprise SSO](/docs/guides/customer-managed-sso). For the platform-team workflow, use [Set up SAML SSO](/docs/authserver/saml-sso).
...
The response includes a `setupUrl`. Send that URL with your own mailer or admin workflow. The first browser open consumes the URL token and stores an HttpOnly server-side portal session cookie. Reuse requires a new setup link. Portal sign-out revokes that server-side session before clearing the cookie, so a retained cookie cannot continue setup.
...
When `RequireSsoForExistingMembers` is enabled, a user who already belongs to the organization and has a verified email on the SSO domain is sent to SSO. The first successful SAML sign-in can link the IdP subject to that existing user only when the connection's organization already owns that email domain. SqlOS does not create a user or membership on this path.

Home realm discovery still follows `RequireSsoForExistingMembers`: when it is disabled, email-first discovery does not force an existing member into SSO. Enable it when existing members should be routed to SSO automatically.

When `AllowJitProvisioning` is enabled, a successful SAML sign-in may create a missing user, or add organization membership for an existing user whose email domain the organization owns. Keep this disabled when access should be invitation- or admin-managed.
...
## Global identity versus organization access

SqlOS users are host-global. A SAML connection is organization-scoped. Those layers are not interchangeable:

| Layer | What it proves | What it does not prove |
| --- | --- | --- |
| Already-bound connection + issuer + subject | That IdP subject is this SqlOS user | Nothing about a different subject or email |
| Active verified organization domain, or operator-managed `PrimaryDomain` | The organization owns that exact email domain | Ownership of another domain or of a global user who merely shares an address |
| Organization membership | Authorization to enter that organization | Control of the host-global identity |
| SCIM provisioning | Directory lifecycle for that organization | That the tenant IdP may bind an unbound global user by email |

A tenant-controlled IdP may authenticate an already-bound subject. It cannot take over an existing global user just by asserting the user's email, even when auto-link, auto-provision, membership, or SCIM records are present. First-time linking of an existing user requires an independently established domain claim: an active `SqlOSOrganizationDomain` for the connection's organization, or that organization's operator-managed `PrimaryDomain`, covering the canonical asserted address exactly (`alice@acme.com` matches `acme.com`, not a sibling or parent domain). Pending or revoked domain claims are ignored.

SCIM remains the right control for who is in the organization. After SCIM creates the membership, the first SAML sign-in still needs that domain ownership before SqlOS will persist the IdP subject on the existing user. Contractors and other external-domain identities are not auto-linked. Keep them on invitation plus a local first factor, or use an IdP subject that is already bound. Existing subject bindings continue to sign in.

Denied first-links are audited as `user.login.saml.link_denied` with a reason code and connection id. Assertion attributes, email addresses, and NameIDs are not written to the audit payload.

**Migration:** identities already stored in `SqlOSExternalIdentities` keep working through subject lookup. New first-time email links of existing users fail closed until the organization owns the asserted domain. Review unexpected connection-subject bindings if a tenant admin may have linked another organization's users before this change.
...
| POST | `/signout` | Revoke the current portal session and clear its cookie |
