...
The default. SqlOS renders the sign-in, sign-up, invite acceptance, OTP, and organization selection pages.
...
- Password, Email OTP, OIDC, and SAML support
...
- Email OTP, invitations, OIDC, and SAML still handled by SqlOS
...
Headless uses one switch: `BuildUiUrl`. If that callback exists, `/sqlos/auth/authorize` redirects into your app. [Details](/docs/authserver/headless-auth#how-sqlos-picks-the-ui).
...
You can flip hosted ↔ headless. OAuth, tokens, sessions, OTP, invitations, and refresh stay the same. Only the login UI changes.

How-to: [Headless Auth](/docs/authserver/headless-auth) · [Email Invitations](/docs/authserver/invitations)
