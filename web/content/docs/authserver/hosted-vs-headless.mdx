...
Headless also needs the right value in the **database**. Use `SeedAuthPage` and `page.PresentationMode = "headless"`. Or leave `PresentationMode` unset if `BuildUiUrl` is set (seed infers headless). [Details](/docs/guides/authserver/headless-auth#presentation-mode).
