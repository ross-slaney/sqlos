...
Use magic links when `model.settings.enabledCredentialTypes` includes `magic_link`. Start sends the email link; complete consumes the token from the link and returns the same redirect, organization, or MFA states as other local credentials.

```tsx
await postAction("/oauth/headless/magic-link/start", {
  requestId: model.requestId,
  email,
  invitationToken,
});

await postAction("/oauth/headless/magic-link/complete", {
  requestId: model.requestId,
  token,
});
```

Magic links are a primary credential, not an MFA factor. If MFA is required, render the returned MFA view before the authorization code is issued.
...
6. Post user actions back to SqlOS
7. Follow redirect results
8. Preserve invitation, challenge, signup, and pending-org tokens exactly as returned
9. Optionally implement `OnHeadlessSignupAsync(...)` for app-specific fields
