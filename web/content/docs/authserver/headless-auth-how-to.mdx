...
Step-by-step companion to [Headless Auth](/docs/guides/authserver/headless-auth).
...
4. Your page loads state from `/headless/requests/{id}`.
...
Set `BuildUiUrl`. Example API: [Program.cs](/Users/employment/Dev/sqlos/examples/SqlOS.Example.Api/Program.cs#L86):
...
- Browser still hits SqlOS `GET /authorize`.
- SqlOS redirects to your page when it needs UI.
- Query params carry enough state to continue.

EasyCOI uses the same pattern with extra query context [here](/Users/employment/Dev/easycoi/api/Program.cs#L135):
