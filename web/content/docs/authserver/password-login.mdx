...
`null`. Returned links must be absolute HTTP or HTTPS URLs without user information, and the token
cannot appear in the URL authority. Invalid link generation fails closed: no email is sent and the
new reset token is invalidated.
