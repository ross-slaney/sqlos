...
A custom AuthPage UI should use [`@sqlos/headless`](/docs/reference/headless-js) (`flow.identify` then `flow.password.login`; recovery is `flow.password.forgot` / `flow.password.reset`). The initial email screen is the `login` view. Always leave a completed flow with `window.location.assign(redirectUrl)`. See [Build your own login and signup UI](/docs/guides/custom-login-ui).

Headless and API-owned UIs can request the same email without exposing the token. The headless routes below are the HTTP wire contract; client code should use the package:
