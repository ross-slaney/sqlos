...
JWKS validation in a separate service proves signature, issuer, JWT lifetime, and audience, but it cannot query the SqlOS session row. Logout or session revocation therefore does not invalidate an already-issued JWT in that service before `exp`. If immediate revocation is a requirement, add an application-owned session/introspection call to a trusted SqlOS host (with caching and fail-closed behavior appropriate to your availability model), or use the SqlOS session-aware validator in the same process.
