...
User access tokens carry no `scope` claim by design: scope constrains what a client may request, while FGA authorizes what a subject may do. Service tokens from client-credentials machine clients are the exception and carry `scope` plus `token_kind: "service"`. See [Scopes and Permissions](/docs/authserver/scopes-and-permissions).
