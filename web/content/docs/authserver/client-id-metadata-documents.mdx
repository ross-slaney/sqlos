...
`CIMD` is disabled by default. With `UseSingleApplication` or `ConfigureApplication`, `app.Mcp = "/mcp"` turns it on together with resource indicators, because portable clients that speak MCP need both. On a host that does not declare `Mcp`, set the flags:
