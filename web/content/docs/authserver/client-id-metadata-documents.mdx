...
`CIMD` is enabled in the raw multi-client defaults; `UseSingleApplication` disables it. The helper below opts a single-application host back into portable-client behavior and also turns on resource indicators:
...
<Callout type="warning" title="CIMD performs a server-side fetch">
  When `TrustedHosts` is empty, any syntactically valid HTTPS client-ID URL with a non-root path is eligible for an outbound metadata request. In production, disable CIMD when portable clients are unnecessary. Otherwise, configure `TrustedHosts` as a pre-fetch allowlist and apply outbound DNS/IP controls that prevent access to loopback, link-local, metadata-service, and private network targets. SqlOS's named CIMD client does not follow HTTP redirects.
</Callout>
...
<Callout type="warning" title="Trust policy is not an egress boundary">
  In the current runtime, `TrustedHosts` and `TrustPolicy` determine whether fetched metadata is accepted, but they run after the outbound request. Constrain DNS/IP destinations and redirects at the network or `HttpClient` layer so CIMD cannot reach loopback, link-local, private, or cloud metadata-service addresses. Use a preregistered client when that egress boundary is unavailable.
</Callout>
