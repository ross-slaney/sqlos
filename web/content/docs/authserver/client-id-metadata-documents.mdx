...
`CIMD` is disabled by default. With `UseSingleApplication` or `ConfigureApplication`, declaring an MCP surface (`app.Mcp = "/mcp"`) enables it together with resource indicators, because portable MCP clients need both. Otherwise the helper below explicitly enables portable-client behavior and resource indicators without requiring a host allowlist:
