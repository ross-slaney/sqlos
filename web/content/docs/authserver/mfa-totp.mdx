...
Primary authentication alone cannot issue an authorization code, AuthPage session, access token, refresh token, or device approval. Email OTP, password, invitation, and ordinary upstream OIDC or SAML do not count as MFA. Phone OTP satisfies MFA only when `PhoneOtp.SatisfiesMfa` is enabled. Explicitly trusted upstream MFA remains accepted. If policy becomes stricter between the first factor and issuance, SqlOS re-evaluates and refuses to issue.
