...
Operator sessions are bound to the configured password. Rotating `Dashboard.Password` invalidates every outstanding operator session cookie, which makes rotation the incident-response step for a leaked cookie or an unattended workstation; rotating the Data Protection key ring has the same effect. Password comparison and the session binding never write the password to the cookie, logs, or audit events.
...
For the end-to-end workflow, see [Brand hosted authentication and email](/docs/guides/auth-branding). Exact feature references remain in [Email OTP](/docs/authserver/email-otp), [Email Invitations](/docs/guides/invite-only-b2b-onboarding), and [Email Branding](/docs/guides/auth-branding).
...
For setup details, see [MFA and TOTP](/docs/authserver/mfa-totp) and [Require Authenticator MFA](/docs/authserver/mfa-totp).
