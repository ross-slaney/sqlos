...
builder.AddSqlOS<AppDbContext>(
    ...
    options => options.UseSingleApplication("Todo", app =>
    {
        ...
        app.Mcp("/mcp", mcp => mcp.WithTools<TodoTools>());      // MCP server (SqlOS.Mcp)
...
| `Mcp("/mcp", ...)` | Resource id `{Origin}/mcp`, `/.well-known/oauth-protected-resource/mcp`, CIMD + resource indicators, and a hosted MCP server. `SqlOS.Mcp` calls `RequireAuthorization("SqlOS.Mcp")` on the endpoint it maps. See [MCP server](/docs/authserver/mcp-server). |
