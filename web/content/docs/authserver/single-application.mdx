...
The same description can carry the application's protected surfaces, branding, and authorization model. SqlOS derives every protocol consequence, so `Program.cs` contains no `AddMcpServer`, `MapMcp`, or hand-written protected-resource document:
...
| --- | --- |
| `Api = "/api"` | Validates bearer tokens for the audience `{Origin}/api` on mapped endpoints under `/api` (401 with a Bearer challenge otherwise), serves `/.well-known/oauth-protected-resource`, and uses `{Origin}/api` as the first-party client's default audience. |
...
`AddSqlOS` attaches bearer validation to the endpoints you map under `Api` or `Mcp`. There is no SqlOS middleware in the host pipeline and no CORS handling: your `UseCors`, `UseAuthentication`, and the rest of the pipeline run first, then the selected endpoint checks the token. A missing or wrong-audience token is answered with `401` and a Bearer challenge. Handlers read the result through `HttpContext.GetSqlOSValidatedToken()` and `HttpContext.User`. Unmatched paths are 404s; raw middleware branches are not endpoints and are not protected.
