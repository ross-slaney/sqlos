...
`Api` and `Mcp` are separate audiences under the same origin. The first-party client receives `{Origin}/api` tokens by default; MCP clients request `{Origin}/mcp` through the `resource` parameter. Both surfaces share the auth server, sessions, users, organizations, and FGA model. Lock API routes with `RequireAuthorization()`. Lock `MapMcp` with `RequireAuthorization("SqlOS.Mcp")`. See [how tokens are validated](/docs/authserver/single-application#how-tokens-are-validated).
