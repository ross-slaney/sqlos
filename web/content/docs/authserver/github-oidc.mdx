...
Creation enables the connection. Use this endpoint only to re-enable one that an operator disabled:
...
See [Authenticate operator API calls](/docs/authserver/dashboard-overview#authenticate-operator-api-calls) before using an Admin API example.
