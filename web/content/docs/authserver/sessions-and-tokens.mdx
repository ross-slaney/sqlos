...
Each login creates an OAuth session (access token plus refresh token) and, for a hosted or headless browser flow, a separate AuthPage session cookie. Clearing the tokens does not end the cookie. See [Refresh and Logout](/docs/authserver/refresh-and-logout).
