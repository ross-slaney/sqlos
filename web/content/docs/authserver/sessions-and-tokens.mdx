...
Each login creates an OAuth session (access token plus refresh token) and, for a hosted or headless browser flow (including system-browser and device-approval sign-in for mobile, desktop, and CLI clients), a separate **issuer session**. Clearing the tokens does not end the issuer session. See [Refresh and Logout](/docs/authserver/refresh-and-logout).

The issuer session is the cookie-backed sign-in at the SqlOS issuer itself: the `sqlos_auth_page` cookie on the identity host that `/authorize` consults to decide whether this browser is already signed in. It is what makes first-party silent reuse, `prompt=none`, `max_age`, `login_required`, and headless `/device` approval possible. Hosted AuthPage and headless custom UIs establish and read the same issuer session; only the login UI differs. It is distinct from the per-client OAuth access/refresh session your application holds. Issuer session cookies that belong to the same logical session share a revocation lifetime: logout invalidates the current cookie and any superseded predecessor from silent renewal.
...
- hosted issuer session cookie reuse
...
Until the browser user approves the CLI request, `/sqlos/auth/token` returns `authorization_pending`. After approval it returns the normal OAuth token response and consumes the device code exactly once. See [CLI OAuth](/docs/guides/terminal-auth).
...
`GET /sqlos/auth/logout` ends one logical issuer session, including superseded cookies from renewal. `LogoutAllAsync` invalidates every issuer session for the user as well as OAuth sessions and refresh-token families. A successful password reset does the same. Organization deactivation invalidates organization-bound OAuth and issuer sessions and revokes organization-owned calendar connections, and the SSO portal's organization-session revocation action invalidates matching-domain issuer sessions along with OAuth sessions. Logout-all and password reset are sign-out operations: they leave [calendar connections](/docs/guides/calendar-integration#offboarding-and-revocation) in place, whereas user or organization deactivation revokes them. Already-issued issuer session cookies that predate session families are consumed when that schema is applied.
