...
Headless apps should treat invitations as a first-class view, not as a generic signup detour. Drive it with [`@sqlos/headless`](/docs/reference/headless-js) (`flow.invitation.resolve` / `flow.invitation.signup`). The routes below are the HTTP wire contract; client code should use the package. See [Build your own login and signup UI](/docs/guides/custom-login-ui).
...
2. Your authorize page resolves the token with `flow.invitation.resolve({ invitationToken })`.
3. Start a normal `/sqlos/auth/authorize` request with `invitationToken` (OIDC library), then `flow.resume(window.location)` — or native `flow.start({ invitationToken })`.
4. Render the returned invite view (`invite`, `invite-login`, `invite-email-otp-verify`, or `invite-accepted`).
5. Existing users continue through `flow.identify`, SSO, password, or OTP sign-in.
6. New invited users with Email OTP enabled call `flow.invitation.signup` directly.
7. When `status === "redirect"`, leave with `window.location.assign(redirectUrl)`. This may be an IdP URL when the invited email is governed by SSO.

<Callout type="warning" title="Do not start OTP signup for invite-created accounts">
  For a new invited user, call `flow.invitation.signup`. Calling `flow.emailOtp.signupStart` creates a second proof step and can strand the invite flow behind an unnecessary code challenge.
</Callout>
