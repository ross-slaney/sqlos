...
The AppHost uses a persistent SQL Server container and passes the database connection string to the API. SqlOS initializes its schema, protected signing key, OAuth client, and FGA seed data when the API starts.
...
- store database credentials, dashboard credentials, and provider secrets outside source control;
