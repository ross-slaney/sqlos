...
The AppHost uses a persistent SQL Server container and passes the database connection string to the API. In Development, the API also persists its Data Protection key ring under the current user's local application-data directory, outside the SQL database. SqlOS initializes its schema, custodied signing key, OAuth client, and FGA seed data when the API starts.
...
### Signing-key custody fails at startup

The development key ring must remain readable by the current user. Outside Development, configure `SqlOS:SigningKeyCustody:DataProtectionKeyRingPath` with a protected, durable path shared by every API instance; the sample deliberately refuses an implicit container-local ring.
...
- use shared, durable, protected Data Protection storage or replace the built-in provider with KMS, Vault, or HSM custody;
