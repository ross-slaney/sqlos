...
>
  Install SqlOS 7.2.0, register one SQL Server or PostgreSQL `DbContext`, and describe one application in one `AddSqlOS` call. This is the standard SqlOS flow; FGA modeling and multi-application client registration can wait.
</Banner>
...
SqlOS 7.2.0 targets `net9.0`; it is not compatible with a `net8.0` host.
...
`SqlOSDbContext<TContext>` registers the AuthServer, calendar, email, and FGA EF models. `UseSingleApplication` creates the application seed, AuthPage branding, and the `{Origin}/api` resource id without exposing client-registration terminology in the common path. Lock `/api/me` with `RequireAuthorization()`. Every line inside `app => { … }` except `Origin` is optional; the full option table is on [Getting started](/docs/getting-started#every-option-you-need-on-one-screen).
...
- [Invite organization members](/docs/guides/invite-only-b2b-onboarding)
