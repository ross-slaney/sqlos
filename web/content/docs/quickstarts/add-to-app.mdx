...
dotnet user-secrets set "SqlOS:SigningKeyCustody:DataProtectionKeyRingPath" "<durable-local-path-outside-the-repository>"
```
...
On the first start, SqlOS creates or upgrades its schema, creates a signing key protected by the configured custody boundary, writes default settings, and reconciles the `acme-web` application seed.
...
### Signing-key custody is not configured

The key-ring path must survive process restarts and must not be the application database or a source-controlled directory. Every token-issuing replica must use the same protected storage and `SetApplicationName` value. Do not set `DataProtectionKeyRingIsPersistedAndShared` merely to bypass the startup check.
...
- Persist, protect, back up, and share ASP.NET Core Data Protection keys across replicas, or register KMS, Vault, or HSM custody. See [Signing-Key Custody](/docs/authserver/signing-key-custody).
