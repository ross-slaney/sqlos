...
>
  Install SqlOS 7.0.0, register one SQL Server or PostgreSQL `DbContext`, and describe one application in one `AddSqlOS` call. This is the standard SqlOS flow; FGA modeling and multi-application client registration can wait.
</Banner>
...
SqlOS 7.0.0 targets `net9.0`; it is not compatible with a `net8.0` host.
