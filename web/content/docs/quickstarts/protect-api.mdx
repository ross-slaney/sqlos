...
>
  Declare `app.Api = "/api"` inside `UseSingleApplication`. SqlOS validates signature, lifetime, audience, and persisted session existence/revocation/absolute expiry on mapped endpoints under `/api` before the handler runs.
</Banner>
...
Declaring `Api` makes SqlOS validate mapped endpoints under `/api` for the audience `http://localhost:5050/api`, use that audience for the `acme-web` client, serve `/.well-known/oauth-protected-resource`, and answer unauthenticated requests with a Bearer challenge that names `realm="Acme API"` and the metadata URL. `AddSqlOS` attaches that validation to those endpoints; nothing is placed or ordered in `Program.cs`. See [how the surfaces are protected](/docs/authserver/single-application#how-the-surfaces-are-protected). After validation it sets `HttpContext.User` and stores the typed `SqlOSValidatedToken`, available through `GetSqlOSValidatedToken()`.
...
Confirm the endpoint's path is under the declared `Api` prefix and import `SqlOS.AuthServer.Extensions`.
...
- For an MCP surface, declare `app.Mcp("/mcp", ...)` next to `Api`; see [MCP server](/docs/authserver/mcp-server).
