...
>
  Apply `RequireSqlOSAccessToken` to a route group. SqlOS validates signature, lifetime, audience, and persisted session existence/revocation/absolute expiry before your endpoint runs.
</Banner>
...
When the API also serves delegated (especially third-party) clients, add `options.RequiredScopes = ["your.scope"]` to enforce the client's granted ceiling — a token whose `scope` claim misses a required entry answers `403 insufficient_scope`. Scope bounds the *client*; per-user, per-resource decisions stay with FGA. See [Scopes and Permissions](/docs/authserver/scopes-and-permissions).
