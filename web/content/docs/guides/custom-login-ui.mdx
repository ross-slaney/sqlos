...
That `scope` value must also appear on the client's `AllowedScopes`. An empty allowlist does not mean "any scope." Omit `scope` on `/authorize` and the grant is empty even when the rest of headless login succeeds. The headless view model includes that granted string as `scope` so the custom UI can show it. That remains valid OAuth. When the client allowlists `openid` and the grant omits it, the view model's `info` and `omittedOpenId` fields warn that an ID token will not be issued once OpenID Provider mode ships. `openid` is never always-allowed. User access tokens still authorize APIs by audience and session, not by this string.
