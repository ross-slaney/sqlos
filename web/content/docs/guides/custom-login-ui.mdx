...
The default JSON API is `/sqlos/auth/headless`. `HeadlessApiBasePath` can move it, and `EnableApi = false` removes it. The returned view model includes its effective `headlessApiBasePath`; use that value when your deployment customizes the route.
...
```ts
const AUTH = "https://identity.relaydesk.example/sqlos/auth";
...
async function loadRequest(route: URLSearchParams): Promise<HeadlessViewModel> {
  ...
  const url = new URL(`${AUTH}/headless/requests/${requestId}`);
  ...
  return response.json() as Promise<HeadlessViewModel>;
}
```
