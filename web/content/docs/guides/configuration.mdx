...
builder.AddSqlOS<AppDbContext>(
    ...
    options => options.UseSingleApplication("Main Web App", app =>
    {
...
Declare `app.Api = "/api"` when your API accepts SqlOS-issued tokens, then lock the group with `RequireAuthorization()`. `HttpContext.User` is populated after authentication; your endpoint still resolves the application subject explicitly.
...
| **CIMD** | Public clients discovered via a metadata URL |
...
Password-mode dashboard mutations and SSO-portal mutations that present their session cookie also require a same-origin browser proof: the `X-SqlOS-Request: 1` header plus an `Origin` or `Referer` that matches `AuthServer.PublicOrigin`, the issuer origin, or the request host after trusted forwarded headers. The hosted dashboard and SSO setup portal send that header themselves and do not store it in browser storage. A missing, conflicting, or cross-origin value is rejected before the mutation or its success audit is committed. Calls that authenticate with a bearer token or `Dashboard.AuthorizationCallback` and do not send either session cookie are unchanged.
