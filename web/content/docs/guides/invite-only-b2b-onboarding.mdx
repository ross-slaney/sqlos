...
Invitation creation is an administrative product action. Declare `app.Api = "/api"` so mapped team routes are validated before the handler runs:
...
SqlOS validates the signature, issuer, audience, token lifetime, and current session. It then makes the validated `UserId`, `OrganizationId`, and `SessionId` available through `GetSqlOSValidatedToken()`.
