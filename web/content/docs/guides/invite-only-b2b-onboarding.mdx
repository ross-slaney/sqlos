...
<Banner
  ...
  href="/docs/guides/invite-only-b2b-onboarding"
...
Invitation creation is an administrative product action. Declare `app.Api = "/api"` and lock the team group with `RequireAuthorization()`:
...
The `SqlOS` scheme validates the signature, issuer, audience, token lifetime, and current session. It then makes the validated `UserId`, `OrganizationId`, and `SessionId` available through `GetSqlOSValidatedToken()`.
...
A custom invitation UI uses the same server-owned state machine. Drive it with [`@sqlos/headless`](/docs/reference/headless-js) (`flow.invitation.resolve` / `flow.start` or `flow.resume` / `flow.invitation.signup`). The routes below are the HTTP wire contract; client code should use the package. See [Build your own login and signup UI](/docs/guides/custom-login-ui).
...
await flow.start({ invitationToken, view: "login" });
// or, after the OIDC library starts /authorize: await flow.resume(window.location);
...
Keep the invited email read-only. Once SqlOS binds the invitation to an authorization request, keep one flow object for that request. Do not replace the invitation flow with `flow.emailOtp.signupStart`; that creates an unnecessary second proof and can lose the bound membership context.
...
- [Email Invitations](/docs/guides/invite-only-b2b-onboarding)
...
- [Transactional Email](/docs/guides/transactional-email)
- [Email Branding](/docs/guides/auth-branding)
