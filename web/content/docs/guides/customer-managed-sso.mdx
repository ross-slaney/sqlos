...
This guide uses the bundled hosted portal. If your platform team owns the entire setup, use [Set up SAML SSO](/docs/authserver/saml-sso) instead. The [SAML reference](/docs/authserver/saml-sso) documents every route and contract after you choose a setup model.
...
SqlOS stores hashes of both the one-time link token and the continuation token. The first successful open consumes the link and sets the `sqlos_sso_portal` cookie. The cookie is HttpOnly, `SameSite=Lax`, scoped to `/sqlos/admin/auth/sso-portal`, marked Secure on HTTPS, bounded by the link expiration, and rejected after the configured idle timeout. Revoking the setup session invalidates its continuation cookie too. Portal mutations that present the cookie, including activation, disablement, and sign-out, also require `X-SqlOS-Request: 1` and a trusted `Origin`. The hosted setup page sends the header and does not store it. A same-site sibling origin is rejected before setup state or a success audit changes. Behind a reverse proxy, the trusted forwarded host must match the origin the browser uses; see [Production Readiness](/docs/guides/production-readiness#reverse-proxies-and-forwarded-headers).

Signing out revokes that exact server-side portal session and records `sso.portal.session.closed` in the same transaction, then clears the continuation cookie with the same HttpOnly, Secure, SameSite, and path attributes. A copied cookie cannot read or change setup state after that commit. Signing out again only clears the cookie. Another portal session for the same organization stays usable until its own sign-out, an operator revoke, or organization deactivation.
...
The policy affects home realm discovery. First-time binding of an existing global user still requires the organization to own the asserted email domain through an active verified domain or operator-managed `PrimaryDomain`.

| Identity state | Conservative defaults | With JIT provisioning on |
| --- | --- | --- |
| Existing active member with a matching email on an owned domain | HRD routes to SSO; the first valid assertion links the IdP subject to the existing user without creating another user or membership | Same result |
| Existing active SqlOS user who is not a member | HRD does not route to this SSO connection, and a connection-directed assertion is denied unless the organization owns the email domain | A valid assertion may add the missing organization membership only when the organization owns that exact email domain |
| Unknown user | HRD does not route to this SSO connection, and a connection-directed assertion is denied | A valid assertion may create a new user, verified email, and membership. It never attaches to a different existing global user by email alone |
...
| Contractor or external-domain identity | Not auto-linked. Use invitation plus a local first factor, or an already-bound subject | Same: domain ownership, not membership, is the first-link proof |

Turn JIT on only when the customer's IdP assignment is authoritative for tenant membership. Keep it off for invitation- or admin-managed onboarding. Turning **Require SSO for existing members** off leaves existing unlinked members on another configured login method; it does not let a tenant IdP take over a global user by email.
...
- Signing out of the portal revokes that one server-side setup session immediately. The predecessor continuation cookie cannot continue setup.
