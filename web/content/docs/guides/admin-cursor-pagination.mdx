...
Application-level authorized lists still use `PagedSpec.For<T>()` with `ISpecificationExecutor`. Admin lists use the shared `SqlOS.Pagination` keyset helper and do not go through `SpecificationExecutor`.
