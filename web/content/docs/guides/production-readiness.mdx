...
`AddSqlOS` installs a startup filter that runs the configured Forwarded Headers middleware before its dashboard middleware. That ordering makes dashboard login throttling, audit events, and cookie CSRF origin checks use the trusted external scheme and host. Configure `ForwardedHeadersOptions` before building the app; you do not need a second `UseForwardedHeaders()` call solely for SqlOS.

Cookie-authenticated dashboard and SSO-portal mutations fail closed unless the browser `Origin` (or `Referer` when `Origin` is absent) matches the configured public origin or that trusted forwarded host. An untrusted `X-Forwarded-Host` does not make an attacker origin valid. Same-origin dashboard and portal pages send `X-SqlOS-Request: 1` with the mutation; programmatic clients that do not present the dashboard or portal session cookie do not need that header.
...
The operator session cookie is bound to the configured dashboard password. Rotating `Dashboard.Password` and restarting or reloading the host invalidates every outstanding operator session immediately, so password rotation is the intended incident-response step when a dashboard cookie or workstation may have been compromised. Cookies issued after the rotation work normally until `SessionLifetime` elapses. Rotating the Data Protection key ring also invalidates outstanding dashboard cookies. There is no separate "sign out all operators" control; every replica must run with the same password so sessions remain valid across instances.
...
- rotate `Dashboard.Password` to revoke outstanding operator sessions after a suspected cookie or workstation compromise;
