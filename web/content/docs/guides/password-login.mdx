...
3. **Wire the client** by following [ASP.NET Core login](/docs/quickstarts/aspnet-core-login). The sample uses the standard ASP.NET Core OAuth handler to create state and PKCE values, redirect through `/authorize`, validate the callback, and exchange the code at `/token`.
