...
The recovery request remains public — do not put `RequireAuthorization()` on those routes. Device management does. Declare `app.Api = "/api"` and lock account endpoints against Harbor's API audience:
...
Do not use user ids from route parameters, forms, query strings, or JSON to decide whose sessions are visible.
...
SqlOS's database-backed `ValidateAccessTokenAsync` and declared `Api`/`Mcp` surfaces check session state. Once a session is revoked, an access token for that session is rejected on the next protected request, and its refresh tokens cannot rotate.
