...
The recovery request remains public — keep those routes outside the declared `Api` prefix. Device management does not. Declare `app.Api = "/api"` and map account endpoints under `/api/account` so every request is validated against Harbor's API audience:
