...
A declared `Api` surface rejects missing, invalid, expired, revoked, or wrong-audience tokens before any enrollment handler runs. If your host already uses ASP.NET Core authentication instead, apply an equivalent authorization policy that validates the exact audience and active SqlOS session.
