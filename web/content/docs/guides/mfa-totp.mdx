...
Hosted auth handles this automatically for password login, OIDC and SAML callbacks, signup, silent SSO, and device approval. Primary authentication cannot issue a code, session, token, or device approval while MFA is required.

For headless auth, render these view names with [`@sqlos/headless`](/docs/reference/headless-js) (`flow.mfa.verify` / `flow.mfa.totp.enrollStart` / `enrollVerify`). The HTTP bodies below are the wire contract; client code should use the package. See [Build your own login and signup UI](/docs/guides/custom-login-ui).
...
- In headless flows, render `mfa` / `mfa-enroll` and call `flow.mfa.verify` or `flow.mfa.totp.enrollVerify`. In direct API flows, treat `requiresMfa` or `requiresMfaEnrollment` as the next auth state. Tokens are intentionally absent until SqlOS completes the second factor.
