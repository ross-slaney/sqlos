...
<Banner
  ...
  href="/docs/guides/service-account-jobs"
...
`SeedMachineClient` and **Auth Server > Machine Clients** create both and link them by `client_id`. The Fine-Grained Auth **Service Accounts** page lists FGA subjects, including ones with no OAuth client. See [Machine Clients](/docs/guides/service-account-jobs) and [Subject types](/docs/fga/subject-types).
...
Exchange it with HTTP Basic. The form field is `resource`, not `audience`, and it must equal the client's registered audience. The token `scope` is the silent intersection of the request and the client allow-list; unknown scopes are dropped rather than rejected:
...
- [Machine Clients](/docs/guides/service-account-jobs)
