...
Build the filter for each authorization request. `BuildFilterAsync` resolves the user's current group subject ids and captures them in the returned expression; caching that expression across membership changes can retain a removed group's id.
