...
On native there is no browser redirect into your page. Drive the flow with [`@sqlos/headless`](/docs/reference/headless-js). `flow.start` creates the authorization request and generates PKCE. Pass `generatePkce` when Web Crypto `subtle` is unavailable (for example Expo). Do not call `/sqlos/auth/headless` from application `fetch` helpers.
...
const flow = createHeadlessFlow({
  ...
  clientId,
  redirectUri,
  generatePkce, // wrap with expo-crypto when Web Crypto subtle is unavailable
});
