...
- [Set up SCIM with Microsoft Entra ID](/docs/guides/scim-microsoft-entra) — attribute mappings, assigned scope, on-demand provisioning, groups, and the recurring job
- [Set up SCIM with Okta](/docs/guides/scim-okta) — app integration, outbound lifecycle switches, assignments, and Group Push
...
For a production rollout, follow the complete [SCIM plus SAML access-lifecycle guide](/docs/guides/scim-saml-access-lifecycle). The short version is that SCIM decides who belongs to the organization, SAML authenticates those already-provisioned people, and SAML JIT provisioning stays off.
...
In the dashboard or delegated SSO portal, the equivalent policy is **Require SSO for existing members = on** and **Allow JIT provisioning = off**. Provision the test user through SCIM before attempting the first SAML sign-in. SqlOS can then bind the IdP subject to that existing member when the organization owns the asserted email domain; it does not treat SCIM or membership as proof of global identity.

That setting gives the smoothest email-first routing for a trusted organization domain. For a narrower routing policy, leave `AutoLinkByEmail` / **Require SSO for existing members** off and have the application start the organization's SAML connection explicitly. First subject link still requires domain ownership.

<Callout type="warning" title="Keep SCIM authoritative">
  Do not enable SAML JIT provisioning when IdP assignment and SCIM deprovisioning are supposed to control access. Otherwise a valid SAML assertion for a not-yet-provisioned person can create a user or membership outside the SCIM assignment lifecycle. First-time SAML linking of an existing global user still requires the organization to own that email domain; SCIM assignment alone is not enough.
</Callout>
...
- [SCIM Directory Sync reference](/docs/guides/scim-directory-sync)
- [SCIM group mapping](/docs/fga/scim-group-mapping)
- [SAML SSO](/docs/authserver/saml-sso)
- [Application Access](/docs/guides/multi-app-access)
