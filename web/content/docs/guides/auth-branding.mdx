...
<Banner
  ...
  href="/docs/guides/auth-branding"
...
Startup seeds claim the matching surface as **code owned** on first reconcile (`auth-page:default` or `auth-email:default`). While that seed remains, dashboard and admin API writes to the same surface are rejected. Removing the seed orphans the record and leaves the last values in place; it does not hand ownership to the dashboard or revert colors, copy, or logos.

AuthPage and email branding are owned independently. You can keep hosted page branding in source control while operators own email branding, or the reverse.

Choose one owner per surface per environment. A common rollout is to seed initial values, remove the seeds after the first deployment, and then let the next dashboard save claim that orphaned or unclaimed surface. If you use `UseSingleApplication`, also disable its implicit branding seeds during that handoff, as shown in [Use the dashboard instead](#5-use-the-dashboard-instead). Do not leave a seed configured if operators should own the surface.

`UseSingleApplication("Acme", ...)` creates code-owned AuthPage and email seeds by default, even when you do not call `SeedAuthPage` or `SeedAuthEmails`. Add explicit seed configuration when you want deliberate colors, layout, copy, or logo values.
...
The dashboard shows a **Code owned** callout and disables the form for any surface still claimed by a startup seed. First dashboard save on an unclaimed `system` default claims that surface for operators.
...
- [Email Branding reference](/docs/guides/auth-branding)
