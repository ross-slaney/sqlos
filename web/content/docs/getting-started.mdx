...
`UseSingleApplication` creates one first-party PKCE client, a callback at `{Origin}/auth/callback`, a default client allowlist (`openid`, `profile`, `email`, `offline_access`), and matching AuthPage branding. Those reserved names stay on the client for gateway compatibility; authorization-server metadata does not advertise them because SqlOS does not yet issue OpenID tokens. Advanced public-client registration and resource indicators remain off until you opt in.
