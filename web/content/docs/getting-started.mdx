...
<Callout type="warning" title="Configuration is explicit">
  `AddSqlOS` does not automatically bind an `SqlOS` configuration section. Read values from `builder.Configuration`, your secret store, or environment variables and assign them in the options callback as shown above. The signing-key flag is an attestation: only set it after Data Protection uses durable, protected storage outside the application database and shared by every token-issuing instance. See [Signing-Key Custody](/docs/authserver/signing-key-custody).
</Callout>
...
When the host starts, SqlOS initializes and upgrades its own tables, verifies signing-key custody, creates a custodied key when none exists, and applies settings and startup seeds. Startup fails closed if custody is unavailable. Your EF migrations continue to own your application tables. `app.MapSqlOS()` maps the OAuth, hosted login, and admin API routes; the dashboard middleware is registered by `AddSqlOS`.
