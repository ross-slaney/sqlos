...
Because application rows and authorization resources share the context, they participate in the same EF save. The application does not create a project successfully and then hope that a second authorization write succeeds later.
...
## The complete runnable application

The following `Program.cs` is a complete .NET 9 minimal API. It creates workspaces, grants the creator a role at the workspace, creates child projects after checking the parent permission, and lists only projects visible to the signed-in subject.
    ...
    CancellationToken cancellationToken) =>
{
    ...
    var workspaceFilter =
        await authorization.GetAuthorizationFilterAsync<Workspace>(
            subjectId,
            createProject);
    ...
    CancellationToken cancellationToken) =>
{
    ...
    var projectFilter =
        await authorization.GetAuthorizationFilterAsync<Project>(
            subjectId,
            readProject);
...
Use the normal SqlOS connection string and dashboard secret configuration:

```json
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=localhost;Database=HierarchicalEf;User Id=sa;Password=LocalDevPassword123!;TrustServerCertificate=True"
  },
  "SqlOS": {
    "Dashboard": {
      "Password": "replace-this-with-a-secret"
    }
  }
}
```
...
6. Apply `GetAuthorizationFilterAsync` to the protected list query.
