...
SqlOS now provides the full **authorization-server side** for preregistered public-client MCP flows. That means branded browser auth, `/sqlos/auth/authorize`, `/sqlos/auth/token`, metadata, JWKS, PKCE, redirect URI validation, sessions, refresh tokens, logout, OIDC, and SAML.
...
- redirect the user to `/sqlos/auth/authorize`
- include `code_challenge`
- use `code_challenge_method=S256`
- exchange the code at `/sqlos/auth/token`
...
{
    options.AuthServer.PublicOrigin = "https://api.example.com";
    options.AuthServer.Issuer = "https://api.example.com/sqlos/auth";
...
- `GET /sqlos/auth/.well-known/oauth-authorization-server`
- `GET /sqlos/auth/.well-known/jwks.json`
- hosted `/sqlos/auth/login`, `/sqlos/auth/signup`, `/sqlos/auth/logout`, and `/sqlos/auth/logged-out`
- public PKCE clients
...
1. Do I expose `/sqlos/auth/.well-known/oauth-authorization-server`?
2. Do I expose JWKS?
